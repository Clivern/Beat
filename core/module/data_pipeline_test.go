...
// TestDataPipelines test cases
func TestDataPipelines(t *testing.T) {
	// Load Configs
	...
	// TestGenerateData
	t.Run("TestGenerateData", func(t *testing.T) {
	...
	t.Run("TestStoreData", func(t *testing.T) {
		...
		err = StoreData(fmt.Sprintf("%s/test01.csv", cacheDir), channel)
		pkg.Expect(t, err, nil)

		// Validate written data
		channel, err = GenerateData(fmt.Sprintf("%s/test01.csv", cacheDir))
		pkg.Expect(t, err, nil)
	...
	t.Run("TestProcessData", func(t *testing.T) {
		...
		err = StoreData(fmt.Sprintf("%s/test02.csv", cacheDir), outChannel)
		pkg.Expect(t, err, nil)

		fileContent, err := util.ReadFile(fmt.Sprintf("%s/test02.csv", cacheDir))
		pkg.Expect(t, err, nil)
